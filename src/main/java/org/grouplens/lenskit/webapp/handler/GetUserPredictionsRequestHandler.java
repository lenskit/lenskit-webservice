...
//Invoked by calling GET /users/[uid]/predictions
	...
	public void handle(Session session, ParsedUrl parsed, HttpServletRequest request, HttpServletResponse response) throws Exception {
		try {
			...
			if (useStored != null && useStored.size() == 1 && useStored.get(0).equalsIgnoreCase("true")) {
				...
				UserPreferencesDto dto = new UserPreferencesDto(Long.toString(userId), ratings.size() + predictions.size(), 0);
				...
				DtoContainer<UserPreferencesDto> responseContainer = new DtoContainer<UserPreferencesDto>(UserPreferencesDto.class, dto);
				...
				DtoContainer<UserPredictionsDto> responseContainer = new DtoContainer<UserPredictionsDto>(UserPredictionsDto.class, dto);
				writeResponse(responseContainer, response, responseFormat);
				response.setStatus(HttpServletResponse.SC_OK);
			}
